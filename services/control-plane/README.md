...
The Runtime Service owns app serving and preview content. When opening the preview root, MoonCraft ensures the project Runtime Service is healthy and initialized, calls Runtime Service `POST /serve`, then proxies the request to the Runtime Service `/preview/` subtree. Successful serve refreshes the Runtime idle TTL. Runtime serve errors are returned to the user and do not refresh the TTL. Generated-application content requests returned by `/preview/...` are proxied as content and do not change preview health.
