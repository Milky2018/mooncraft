...
| TASK-013 | Runtime | Integrate real Runtime project editing behind `RuntimeGateway`. | IN_PROGRESS | Background worker mode, selectable Runtime definitions, persistent Runtime sessions, and validation-before-preview are implemented; Docker/runtime hardening remains. |
...
| TASK-021 | Preview smoke story | The smoke story does not yet assert persistence after project reload for project rail, chat history, and stale preview handling. | DONE | Smoke coverage verifies project detail persistence and fast stale-preview failure without embedding the preview in the platform page. |
