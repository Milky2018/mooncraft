...
```bash
just build-agent-runtime-image docker.io/moonbitcloud/mooncraft-agent-runtime 0.1.4

# Build local architecture-suffixed images for both supported platforms:
just build-agent-runtime-images docker.io/moonbitcloud/mooncraft-agent-runtime 0.1.4

# Publish the shared multi-architecture image:
just docker-agent-runtime-publish docker.io/moonbitcloud/mooncraft-agent-runtime 0.1.4 linux/amd64,linux/arm64
...
`just build-agent-runtime-image` defaults to the current Docker host platform, which is the right choice for local smoke tests. `just build-agent-runtime-images` builds both local platform-specific tags, such as `:0.1.4-amd64` and `:0.1.4-arm64`. Production and shared test deployments should use `just docker-agent-runtime-publish`, which builds and pushes one multi-architecture tag that Docker can resolve by host platform.
...
```bash
docker image rm docker.io/moonbitcloud/mooncraft-agent-runtime:0.1.4 || true
just deploy-prod
```
